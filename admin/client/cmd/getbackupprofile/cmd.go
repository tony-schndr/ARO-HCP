...
func NewGetBackupProfileCommand() (*cobra.Command, error) {
	opts := base.DefaultAuthOptions()
	var subscriptionID, resourceGroup, clusterName string

	cmd := &cobra.Command{
		Use:           "get-backup-profile",
		Short:         "Get backup profile for an HCP cluster",
		SilenceErrors: true,
		SilenceUsage:  true,
...
func execute(ctx context.Context, opts *base.RawAuthOptions, subscriptionID, resourceGroup, clusterName string) error {
	...
	logger.Info("Getting backup profile", "subscriptionID", subscriptionID, "resourceGroup", resourceGroup, "clusterName", clusterName)

	client := adminClient.NewClient(completed.Endpoint, completed.Host, completed.Token, completed.Insecure, false)
	err = client.GetBackupProfile(ctx, subscriptionID, resourceGroup, clusterName)
	if err != nil {
