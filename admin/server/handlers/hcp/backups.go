...
func GetBackup(resourcesDBClient database.ResourcesDBClient, csClient ocm.ClusterServiceClientSpec, azureCredential azcore.TokenCredential, mgmtClientFactory MgmtClientFactory, clock utilsclock.PassiveClock) http.Handler {
	return http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		drCtx, status, err := resolveDRContext(request, resourcesDBClient, csClient, azureCredential, mgmtClientFactory, clock)
		if err != nil {
...
func ListBackups(resourcesDBClient database.ResourcesDBClient, csClient ocm.ClusterServiceClientSpec, azureCredential azcore.TokenCredential, mgmtClientFactory MgmtClientFactory, clock utilsclock.PassiveClock) http.Handler {
	return http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		drCtx, status, err := resolveDRContext(request, resourcesDBClient, csClient, azureCredential, mgmtClientFactory, clock)
		if err != nil {
			http.Error(writer, fmt.Sprintf("failed to resolve DR context: %v", err), status)
...
func CreateBackup(resourcesDBClient database.ResourcesDBClient, csClient ocm.ClusterServiceClientSpec, azureCredential azcore.TokenCredential, mgmtClientFactory MgmtClientFactory, clock utilsclock.PassiveClock) http.Handler {
	return http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		drCtx, status, err := resolveDRContext(request, resourcesDBClient, csClient, azureCredential, mgmtClientFactory, clock)
		if err != nil {
			http.Error(writer, fmt.Sprintf("failed to resolve DR context: %v", err), status)
