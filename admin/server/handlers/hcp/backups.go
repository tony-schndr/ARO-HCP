...
func resolveDRContext(
	...
	clusterServiceID := path.Base(hcp.ServiceProviderProperties.ClusterServiceID.String())
	shardIndex := recovery.AssignShard(clusterServiceID, veleroShardCount)
	veleroNS := recovery.VeleroShardNamespace(shardIndex)

	return &drContext{resourceID: resourceID.String(), client: client, hcp: hcp, time: clock, veleroNamespace: veleroNS}, http.StatusOK, nil
}
...
func getBackup(ctx context.Context, client ctrlclient.Client, backupName, veleroNamespace string) (*velerov1api.Backup, error) {
	backup := &velerov1api.Backup{}
	key := ctrlclient.ObjectKey{Name: backupName, Namespace: veleroNamespace}
...
func listBackupsForCluster(ctx context.Context, client ctrlclient.Client, clusterID, veleroNamespace string) ([]velerov1api.Backup, error) {
	backupList := &velerov1api.BackupList{}
	if err := client.List(ctx, backupList, ctrlclient.InNamespace(veleroNamespace), ctrlclient.MatchingLabels{"api.openshift.com/id": clusterID}); err != nil {
		return nil, err
	}
	return backupList.Items, nil
}

func createBackupForCluster(ctx context.Context, client ctrlclient.Client, clusterID, veleroNamespace, timestamp string) (*velerov1api.Backup, error) {
	...
	ttl := 7 * 24 * time.Hour
	backup := recovery.NewBackup(veleroNamespace, backupName, clusterID, ttl, hc.Namespace, hcpNamespace)
...
func GetBackup(resourcesDBClient database.ResourcesDBClient, fleetDBClient database.FleetDBClient, azureCredential azcore.TokenCredential, mgmtClientFactory MgmtClientFactory, clock utilsclock.PassiveClock, veleroShardCount int) http.Handler {
	return http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		drCtx, status, err := resolveDRContext(request, resourcesDBClient, fleetDBClient, azureCredential, mgmtClientFactory, clock, veleroShardCount)
		if err != nil {
		...
		veleroBackup, err := getBackup(request.Context(), drCtx.client, backupName, drCtx.veleroNamespace)
		if err != nil {
...
func ListBackups(resourcesDBClient database.ResourcesDBClient, fleetDBClient database.FleetDBClient, azureCredential azcore.TokenCredential, mgmtClientFactory MgmtClientFactory, clock utilsclock.PassiveClock, veleroShardCount int) http.Handler {
	return http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		drCtx, status, err := resolveDRContext(request, resourcesDBClient, fleetDBClient, azureCredential, mgmtClientFactory, clock, veleroShardCount)
		if err != nil {
			http.Error(writer, fmt.Sprintf("failed to resolve DR context: %v", err), status)
		...
		clusterServiceID := drCtx.clusterServiceID()
		backups, err := listBackupsForCluster(request.Context(), drCtx.client, clusterServiceID, drCtx.veleroNamespace)
		if err != nil {
...
func CreateBackup(resourcesDBClient database.ResourcesDBClient, fleetDBClient database.FleetDBClient, azureCredential azcore.TokenCredential, mgmtClientFactory MgmtClientFactory, clock utilsclock.PassiveClock, veleroShardCount int) http.Handler {
	return http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		drCtx, status, err := resolveDRContext(request, resourcesDBClient, fleetDBClient, azureCredential, mgmtClientFactory, clock, veleroShardCount)
		if err != nil {
			http.Error(writer, fmt.Sprintf("failed to resolve DR context: %v", err), status)
		...
		clusterServiceID := drCtx.clusterServiceID()
		backup, err := createBackupForCluster(request.Context(), drCtx.client, clusterServiceID, drCtx.veleroNamespace, drCtx.time.Now().Format("20060102150405"))
		if err != nil {
