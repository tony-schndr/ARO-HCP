...
func GetBackup(dbClient database.DBClient, csClient ocm.ClusterServiceClientSpec, azureCredential azcore.TokenCredential, mgmtClientFactory MgmtClientFactory, clock utilsclock.PassiveClock) http.Handler {
	return http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		drCtx, status, err := resolveDRContext(request, dbClient, csClient, azureCredential, mgmtClientFactory, clock)
		if err != nil {
...
func ListBackups(dbClient database.DBClient, csClient ocm.ClusterServiceClientSpec, azureCredential azcore.TokenCredential, mgmtClientFactory MgmtClientFactory, clock utilsclock.PassiveClock) http.Handler {
	return http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		drCtx, status, err := resolveDRContext(request, dbClient, csClient, azureCredential, mgmtClientFactory, clock)
		if err != nil {
			http.Error(writer, fmt.Sprintf("failed to resolve DR context: %v", err), status)
...
func CreateBackup(dbClient database.DBClient, csClient ocm.ClusterServiceClientSpec, azureCredential azcore.TokenCredential, mgmtClientFactory MgmtClientFactory, clock utilsclock.PassiveClock) http.Handler {
	return http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		drCtx, status, err := resolveDRContext(request, dbClient, csClient, azureCredential, mgmtClientFactory, clock)
		if err != nil {
			http.Error(writer, fmt.Sprintf("failed to resolve DR context: %v", err), status)
		...
		id := drCtx.clusterServiceID()
		backup, err := createBackupForCluster(request.Context(), drCtx.client, id, drCtx.time.Now().Format("20060102150405"))
		if err != nil {
