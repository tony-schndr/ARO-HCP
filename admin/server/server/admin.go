...
func NewAdminAPI(
	...
	middlewareMux.Handle(
		...
		hcpMiddleware.Handler(hcp.ListBackups(resourcesDBClient, clustersServiceClient, azureCredential, mgmtClientFactory, clock)),
	)
	middlewareMux.Handle(
		middleware.V1HCPResourcePattern("GET", "/backups/{backupName}"),
		hcpMiddleware.Handler(hcp.GetBackup(resourcesDBClient, clustersServiceClient, azureCredential, mgmtClientFactory, clock)),
	)
	middlewareMux.Handle(
		middleware.V1HCPResourcePattern("POST", "/backups"),
		hcpMiddleware.Handler(hcp.CreateBackup(resourcesDBClient, clustersServiceClient, azureCredential, mgmtClientFactory, clock)),
	)
	middlewareMux.Handle(
