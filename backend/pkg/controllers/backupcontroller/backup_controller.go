...
func (c *backupScheduleSyncer) SyncOnce(ctx context.Context, key controllerutils.HCPClusterKey) error {
	...
	hasActiveCreate, err := controllerutils.ClusterHasActiveCreateOperation(ctx, c.activeOperationLister, existingCluster)
	if err != nil {
		return utils.TrackError(fmt.Errorf("failed to check active create operation: %w", err))
	}
	if hasActiveCreate {
		return nil
	}
