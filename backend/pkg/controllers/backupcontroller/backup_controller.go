...
func (c *backupScheduleSyncer) SyncOnce(ctx context.Context, key controllerutils.HCPClusterKey) error {
	...
	shardIndex := recovery.AssignShard(clusterID, c.backupConfig.VeleroShardCount)
	...
	for _, sc := range c.backupConfig.Schedules {
		...
		schedule := NewScheduledBackup(veleroNS, clusterID, clusterName, hcNamespace, hcpNamespace, sc.Name, sc.Schedule, sc.TTLDuration(), paused)
