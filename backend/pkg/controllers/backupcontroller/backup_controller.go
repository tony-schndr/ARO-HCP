...
func (c *backupScheduleSyncer) SyncOnce(ctx context.Context, key controllerutils.HCPClusterKey) error {
	...
	ddCrud, err := kaClient.DeleteDesiresForCluster(key.SubscriptionID, key.ResourceGroupName, key.HCPClusterName)
	if err != nil {
		return utils.TrackError(fmt.Errorf("get DeleteDesire CRUD: %w", err))
	}

	if err := c.deleteStaleDesires(ctx, adCrud, rdCrud, ddCrud, mcResourceID, desiredADs); err != nil {
		return err
	}
...
func (c *backupScheduleSyncer) deleteStaleDesires(
	...
	adCrud database.ResourceCRUD[kubeapplier.ApplyDesire, *kubeapplier.ApplyDesire],
	rdCrud database.ResourceCRUD[kubeapplier.ReadDesire, *kubeapplier.ReadDesire],
	ddCrud database.ResourceCRUD[kubeapplier.DeleteDesire, *kubeapplier.DeleteDesire],
	...
	for _, ad := range iterator.Items(ctx) {
		...
		deleteDesire, err := buildDeleteDesireFromApplyDesire(ad, mcResourceID)
		if err != nil {
			return utils.TrackError(fmt.Errorf("failed to build DeleteDesire for stale schedule %s: %w", name, err))
		}
		if _, err := ddCrud.Create(ctx, deleteDesire, nil); err != nil && !database.IsConflictError(err) {
			return utils.TrackError(fmt.Errorf("failed to create DeleteDesire for stale schedule %s: %w", name, err))
		}

		if err := adCrud.Delete(ctx, name); err != nil && !database.IsNotFoundError(err) {
			return utils.TrackError(fmt.Errorf("failed to delete stale ApplyDesire %s: %w", name, err))
		}
...
// applyDesireNeedsUpdate reports whether the existing ApplyDesire's KubeContent
// differs from the desired one.
func applyDesireNeedsUpdate(existing, desired *kubeapplier.ApplyDesire) bool {
	if existing == nil {
		return true
	}
	existingRaw := existing.Spec.KubeContent
	desiredRaw := desired.Spec.KubeContent
	...
	if err := json.Unmarshal(existingRaw.Raw, &existingObj); err != nil {
		return true
	}
	if err := json.Unmarshal(desiredRaw.Raw, &desiredObj); err != nil {
		return true
	}
