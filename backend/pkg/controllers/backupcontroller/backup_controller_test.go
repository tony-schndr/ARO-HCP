...
func TestBackupScheduleSyncer_SyncOnce(t *testing.T) {
	...
	// seedClusterCreateOperation seeds a non-terminal Create operation into the
	// mock DB so the DB-backed active operation lister can find it.
	seedClusterCreateOperation := func(t *testing.T, ctx context.Context, mockDB *databasetesting.MockResourcesDBClient, opName string) {
		t.Helper()
		clusterResourceID := api.Must(api.ToClusterResourceID(testKey.SubscriptionID, testKey.ResourceGroupName, testKey.HCPClusterName))
		opResourceID := api.Must(azcorearm.ParseResourceID(api.ToOperationResourceIDString(testKey.SubscriptionID, opName)))
		...
		_, err := mockDB.Operations(testKey.SubscriptionID).Create(ctx, op, nil)
		require.NoError(t, err)
	}
	...
	tests := []struct {
		...
		{
			name: "cluster with active create operation in flight is skipped",
			seedDB: func(t *testing.T, ctx context.Context, mockDB *databasetesting.MockResourcesDBClient) {
				t.Helper()
				_, err := mockDB.HCPClusters(testKey.SubscriptionID, testKey.ResourceGroupName).Create(ctx, newTestCluster(func(c *api.HCPOpenShiftCluster) {
					c.ServiceProviderProperties.ActiveOperationID = "op-create-1"
				}), nil)
				require.NoError(t, err)
				seedSPCWithPlacement(t, ctx, mockDB)
				seedClusterCreateOperation(t, ctx, mockDB, "op-create-1")
			},
			verify: func(t *testing.T, ctx context.Context, _ *databasetesting.MockResourcesDBClient, mockKA *databasetesting.MockKubeApplierDBClient) {
				t.Helper()
				adCrud, err := mockKA.ApplyDesiresForCluster(testKey.SubscriptionID, testKey.ResourceGroupName, testKey.HCPClusterName)
				...
				for _, sc := range testBackupConfig.Schedules() {
					desireName := backupApplyDesireName(fmt.Sprintf("%s-%s", testClusterID, sc.Name))
					_, err := adCrud.Get(ctx, desireName)
					assert.True(t, database.IsNotFoundError(err), "desire %s should not exist while create is in flight", desireName)
				}
			},
		},
		{
