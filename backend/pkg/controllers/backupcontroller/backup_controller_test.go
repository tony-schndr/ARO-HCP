...
func TestNewScheduledBackup(t *testing.T) {
	...
	schedule := NewScheduledBackup("velero-0", clusterID, clusterName, hcNamespace, hcpNamespace, "hourly", cronSchedule, ttl, false)

	assert.Equal(t, clusterID+"-hourly", schedule.Name)
	assert.Equal(t, "velero-0", schedule.Namespace)
	...
	pausedSchedule := NewScheduledBackup("velero-0", clusterID, clusterName, hcNamespace, hcpNamespace, "hourly", cronSchedule, ttl, true)
	assert.Equal(t, true, pausedSchedule.Spec.Paused)

	// Verify different schedule names produce different Velero Schedule names
	dailySchedule := NewScheduledBackup("velero-0", clusterID, clusterName, hcNamespace, hcpNamespace, "daily", "0 2 * * *", 48*time.Hour, false)
...
func TestBuildScheduleManifestWork(t *testing.T) {
	...
	hourlySchedule := NewScheduledBackup("velero-0", clusterID, "test-domprefix", hcNamespace, hcpNamespace, "hourly", "0 */1 * * *", 48*time.Hour, false)
	dailySchedule := NewScheduledBackup("velero-0", clusterID, "test-domprefix", hcNamespace, hcpNamespace, "daily", "0 2 * * *", 336*time.Hour, false)
	weeklySchedule := NewScheduledBackup("velero-0", clusterID, "test-domprefix", hcNamespace, hcpNamespace, "weekly", "0 3 * * 0", 2160*time.Hour, false)
	...
	for i, s := range schedules {
		...
		assert.Equal(t, "velero-0", mc.ResourceIdentifier.Namespace)
...
func TestBackupScheduleSyncer_SyncOnce(t *testing.T) {
	...
	buildExpectedMW := func() *workv1.ManifestWork {
		...
		veleroNS := recovery.VeleroShardNamespace(recovery.AssignShard(testClusterID, testBackupConfig.VeleroShardCount))
		schedules := make([]*velerov1api.Schedule, 0, len(testBackupConfig.Schedules))
		for _, sc := range testBackupConfig.Schedules {
			schedules = append(schedules, NewScheduledBackup(veleroNS, testClusterID, testDomainPrefix, hcNamespace, hcpNamespace, sc.Name, sc.Schedule, sc.TTLDuration(), false))
		}
		mw, err := buildScheduleManifestWork(
...
func TestEnsureManifestWorkPatched_IdempotentAfterRoundTrip(t *testing.T) {
	...
	hcNamespace := "ocm-testenv-" + clusterID
	hcpNamespace := hcNamespace + "-test-domprefix"
	schedule := NewScheduledBackup("velero-0", clusterID, "test-domprefix", hcNamespace, hcpNamespace, "hourly", "0 */1 * * *", 48*time.Hour, false)
