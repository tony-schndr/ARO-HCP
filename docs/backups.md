...
3. **Delete stale desires** — If there are ApplyDesires with the `backup-` prefix that no longer correspond to a configured schedule, create a DeleteDesire (to remove the Velero Schedule from the management cluster) and delete the stale ApplyDesire and ReadDesire.
...
| Field | Description                                                                              |
|-------|------------------------------------------------------------------------------------------|
| `paused` | When `true`, pauses all backup schedules for all clusters.                               |
| `cadence` | Either `"production"` or `"testing"`. Controls which set of hardcoded schedules is used. |

Schedules are not individually configurable in the YAML file. They are determined by the cadence:
...
Validation requires `cadence` to be either `"production"` or `"testing"`. See `backend/pkg/controllers/backupcontroller/config.go`.

## Pause and Resume

Backup schedules can be paused at two levels:

- **Global pause** — Set via the `paused` field in the backup config YAML. When `true`, all schedules for all clusters are paused. Requires a backend redeployment to take effect.
...
3. Creates a DeleteDesire (so kube-applier removes the Velero Schedule from the management cluster).
...
Edit the backup config YAML to set `paused: true`. Redeploy the backend service. All clusters will have their schedules paused on the next reconciliation cycle.
