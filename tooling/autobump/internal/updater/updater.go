...
import (
	...
	"github.com/Azure/ARO-HCP/tooling/autobump/internal/clients"
	"github.com/Azure/ARO-HCP/tooling/autobump/internal/config"
	"github.com/Azure/ARO-HCP/tooling/autobump/internal/yaml"
)
...
type ImageUpdate struct {
	Name        string
	NewDigest   string
	Environment string // e.g., "dev", "int"
	FilePath    string
}
...
func (u *Updater) updateImage(ctx context.Context, name string, latestDigest string, target config.Target) error {
	...
	// Determine environment from the jsonPath
	environment := determineEnvironment(target.JsonPath)
...
func (u *Updater) GenerateCommitMessage() string {
	...
	// Output grouped by environment
	for _, updates := range envUpdates {
		if len(updates) > 0 {
...
// determineEnvironment extracts the environment from the jsonPath
func determineEnvironment(jsonPath string) string {
	// Check for explicit environment in path
	if strings.Contains(jsonPath, "clouds.public.environments.int") {
		return "int"
	}
	if strings.Contains(jsonPath, "clouds.dev") {
		return "dev"
	}
	if strings.Contains(jsonPath, "defaults.") {
		return "dev" // default targets are for dev
	}
	return "unknown"
}
